...
def load_data(path):
    df = pd.read_csv(path)
    return df
# cache no need to reload the data all the time, faster to run.

mpg_df_raw = load_data(path="./data/mpg.csv") # ./data, here means go back to last file, and then go to data file.
mpg_df = deepcopy(mpg_df_raw) # everytime copy and get the raw data

# Add title and header
st.title("Introduction to Streamlit") #set the title etc.
...
# if function, when click the sidebar, yes and then show the subheader etc.

# Setting up columns
left_column, middle_column, right_column = st.columns([3, 1, 1])

# Widgets: selectbox
years = ["All"]+sorted(pd.unique(mpg_df['year'])) # all + all years
